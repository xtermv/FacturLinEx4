...
type
  ...
  TfImportador = class(TForm)
    ...
    procedure Formatear();
    procedure DistribuirLineas(var ArrayDeLineasAux: array of RLinea);
    procedure BuscarCoincidencias(var Linea: RLinea);
    procedure CompletaLineaPedidoConBD(var Linea: RLinea);
    procedure WriteLinea(const Linea: RLinea);
    procedure tsPendientesEnter(Sender: TObject);
    procedure tsProcesadosEnter(Sender: TObject);
    procedure NuevoEan(const nuevoEan: string; var linea: RLinea);
...
var
  ...
  //dbPedidAux: TZQuery;
  ColorLineas: TColor;
  ArrayDeLineas: array of RLinea;
  ArrayDeLineasPen: array of RLinea;
  ArrayDeLineasPro: array of RLinea;
...
//procedure TfImportador.FormCreate(Sender: TObject;FPedido: TFPedido);
procedure TfImportador.FormCreate(Sender: TObject);
begin
...
procedure TfImportador.FormShow(Sender: TObject);
begin
...
procedure ShowFormImportar();
begin
  with TfImportador.Create(Application) do
    begin
       //dbPedidAux:= dbPedid;
       //dbPedicAux:= dbPedic;
       ShowModal;
...
//=== Seleccionar el fichero origen de los datos ===
procedure TfImportador.btnSeleccionarClick(Sender: TObject);
var
...
// El formateo se lanza al salir de los edit donde se definen los campos
procedure TfImportador.Formatear();
var
  Linea: RLinea;
  PosFin: integer;
  PosIni: integer;
...
begin
    ...
    if pc.ActivePage=tsSeleccion then begin   // Activada la pestaña de Selección de Posiciones
       ...
       while not EOF(F) do begin
             ...
             BuscarCoincidencias(Linea);  //Busca en la BD y carga los valores de coincidencias
             SetLength(ArrayDeLineas, cont); // Redimensiona el vector al tamaño del contador(líneas contadas)
             ArrayDeLineas[cont-1]:=linea;//Empieza a contar desde 0 y cont se inicializa en 1
...
//-------- Lanza la distribución de líneas ---------
procedure TfImportador.btnGenerarClick(Sender: TObject);
begin
  tsProcesados.Enabled:=true;
  tsPendientes.Enabled:=true;
  pc.ActivePage:=tsPendientes;
  btnAPedido.Visible:=True;
  DistribuirLineas(ArrayDeLineas);
end;

//=============== DISTRIBUIR LINEAS ==========================
//Una vez identificadas las líneas, se clasifican según se hayan
// identificado los artículos implicados, esto lo hacemos trabajando con las
// coincidencias obtenidad en BuscarCoincidencias
procedure TfImportador.DistribuirLineas(var ArrayDeLineasAux: array of RLinea);
var
  cont: integer;
...
begin
  ...
  SetLength(ArrayDeLineasPen, 0);
  SetLength(ArrayDeLineasPro, 0);
  //writeln(Length(ArrayDeLineasAux)-1);
  for cont:=0 to Length(ArrayDeLineasAux)-1 do
  begin
  //WriteLn('Dentro de distribuir:');
  //WriteLinea(ArrayDeLineasAux[cont]);
   //cada línea tendrá 3 integer que indicarán la concordancia
   //de cada campo significativo entre el txt y la bd: CoinCod, CoinEan, CoinDes
                        //0 ausente en el txt,
                        //1 está en artitien
                        //2 está en eans
    ...
    if (
         ( not
               (
                 (ArrayDeLineasAux[cont].CoinCod=3) OR
                 (ArrayDeLineasAux[cont].CoinDes=3) OR
                 (ArrayDeLineasAux[cont].CoinEan=3)
               )
         ) AND
         (
           ((ArrayDeLineasAux[cont].CoinCod<>0) AND (ArrayDeLineasAux[cont].CoinEan<>0)) OR
           ((ArrayDeLineasAux[cont].CoinCod<>0) AND (ArrayDeLineasAux[cont].CoinDes<>0)) OR
           ((ArrayDeLineasAux[cont].CoinEan<>0) AND (ArrayDeLineasAux[cont].CoinDes<>0))
         )
       ) then
      begin
        //Esto es absurdo en este importador, el artículo conocido no hay que tocarlo
        ////Antes de guardarla nos aseguramos que todos los datos son completos y correctos
        ////writelinea(ArrayDeLineasAux[cont]);
        //CompletaLineaPedidoConBD(ArrayDeLineasAux[cont]);
        //writelinea(ArrayDeLineasAux[cont]);
        dimPro:= Length(ArrayDeLineasPro);
        SetLength(ArrayDeLineasPro, dimPro+1);
        ArrayDeLineasPro[dimPro]:=ArrayDeLineasAux[cont];
        LlenarLineaGrid(dbgProcesados, dimPro+1, ArrayDeLineasAux[cont], AnchuraColumnaCod, AnchuraColumnaEan, AnchuraColumnaDes);
      end
    //todo lo demás está pendiente
    else
    begin
      //Aquí tenemos los desconocidos que son los que vamos a importar
      //ShowMessage(inttostr(cont)+'en pendientes');
      dimPen:= Length(ArrayDeLineasPen);
      SetLength(ArrayDeLineasPen, dimPen+1);
      ArrayDeLineasPen[dimPen]:=ArrayDeLineasAux[cont];
      LlenarLineaGrid(dbgPendientes, dimPen+1, ArrayDeLineasAux[cont], AnchuraColumnaCod, AnchuraColumnaEan, AnchuraColumnaDes);
    end;
  end;
end;

//Método inutil en el importador
//==== COMPLETAR LAS LINEAS CON DATOS DE LA BD =====
...
procedure TfImportador.CompletaLineaPedidoConBD(var Linea: RLinea);
begin
...
procedure TfImportador.BuscarCoincidencias(var Linea: RLinea);
begin
  ...
  if (Linea.Codigo<>'') then
     begin
      ...
      dbArti.Active:=True;
      if dbArti.RecordCount<>0 then Linea.CoinCod:=1
      else
        begin
          dbEans.Active:=False;
          dbEans.SQL.Text:='SELECT * FROM eans WHERE EAN0="'+Linea.Codigo+'"';
          dbEans.Active:=True;
          if dbEans.RecordCount<>0 then Linea.CoinCod:=2;
        end;
   ...
   if (Linea.CodigoEAN<>'') then
     begin
      ...
      linea.CoinEan:=3;
       ...
       dbEans.Active:=True;
       if dbEans.RecordCount<>0 then linea.CoinEan:=2
       else
         begin
           dbArti.Active:=False;
           dbArti.SQL.Text:='SELECT * FROM artitien'+Tienda+' WHERE A0="'+linea.CodigoEAN+'"';
           dbArti.Active:=True;
           if dbArti.RecordCount<>0 then linea.CoinEan:=1;
         end;
  ...
  if (Linea.Descripcion <> '') then
    begin
      ...
      dbArti.Active:=True;
      if (dbArti.RecordCount<>0) then Linea.CoinDes:=1 //nombre encontrado en artitien
      else
        begin
          dbEans.Active:=False;
          dbEans.SQL.Text:='SELECT * FROM eans WHERE EAN2="'+Linea.Descripcion+'"';
          dbEans.Active:=True;
          if (dbEans.RecordCount<>0) then Linea.CoinDes:=2;//nombre encontrado en eans
        end;
    end;
end;
...
procedure TfImportador.LlenarLineaGrid(
  var gridDatos: TStringGrid;
  cont: integer;
  const Linea: RLinea;
  var AnchuraColumnaCod: integer;
...
begin
  ...
  //Guarda en el grid la posicion que ocupa la linea en el ArrayDeLineas
  gridDatos.Cells[7, cont]:=intTostr(linea.Pos);
  if (linea.Codigo<>'') then begin
       if (AnchuraColumnaCod<length(linea.Codigo)) then AnchuraColumnaCod:=length(linea.Codigo);
    ...
    if (linea.CodigoEAN<>'') then begin
        if (AnchuraColumnaEan<length(linea.CodigoEan)) then AnchuraColumnaEan:=length(linea.CodigoEan);
    ...
    if (linea.Descripcion <> '') then begin
         if (AnchuraColumnaDes<length(linea.Descripcion)) then AnchuraColumnaDes:=length(linea.Descripcion);
...
//================== TODO EN BLANCO ===========================
procedure TfImportador.TodoEnblanco1();
begin
...
procedure TfImportador.TodoEnBlanco2();
begin
  eDelimitador.Text:=',';
  eOEAN.Text:='';
  eOCodigo.Text:='';
...
procedure TfImportador.sbLimpiar1Click(Sender: TObject);
begin
 TodoEnblanco1();
end;

procedure TfImportador.sbLimpiar2Click(Sender: TObject);
begin
 TodoEnblanco2();
end;

// Carga los parametros de posiciones desde un fichero
procedure TfImportador.sbLoad1Click(Sender: TObject);
var
  F: TextFile;
  Txt: String;
begin
  ...
  if LoadDialog.Execute then begin
     ...
     Reset(F);
     // Usamos IntToStr(StrToInt( )) para eliminar los CEROS.
     //Lo ideal sería una función en *f_cadenas.pas* para eliminar un carácter de una cadena. *** PARA HACER ***.
     ReadLn(F,Txt);
...
// Carga valores de orden y separador desde un fichero
procedure TfImportador.sbLoad2Click(Sender: TObject);
var
  F: TextFile;
  Txt: String;
begin
...
// Guarda los parametros de posiciones en un fichero
procedure TfImportador.sbSave1Click(Sender: TObject);
var
  F: TFileStream;
  s: String;
begin
...
//Guarda valores de orden y separador en un fichero
procedure TfImportador.sbSave2Click(Sender: TObject);
var
  F: TFileStream;
  s: String;
begin
...
// Añade nuevo Ean
procedure TfImportador.NuevoEan(const nuevoEan: string; var linea: RLinea);
begin
...
//Este procedimiento hay que cambiarlo
procedure TfImportador.btnAPedidoClick(Sender: TObject);
var cont:integer;
begin
  for cont:=0 to Length(ArrayDeLineasPro)-1 do
  begin
    // En pedidos empiezan a insertar en la línea 1, hay que sumar 1 al nº de líneas
    //InsertarLinea(ArrayDeLineasPro[cont], (dbPedidAux.RecordCount+1));
...
////----------------- Aceptar Lineas ----------------
////procedure TfImportador.InsertarLinea(var dbPedidI: TzQuery; dbPedicAux: TzQuery; Linea: RLinea; VerUltimaLinea: integer );
//procedure TfImportador.InsertarLinea(Linea: RLinea; VerUltimaLinea: integer );
//begin
//    dbPedidAux.Append;
//    begin
...
//procedure TfImportador.SumaPendientes(CodiPen, UniPen: String);
//begin
...
procedure TfImportador.btnSalirClick(Sender: TObject);
begin
  Self.Close;
end;
...
procedure TfImportador.dbgPendientesDblClick(Sender: TObject);
var
fila:integer;
linea: RLinea;
TxtQuery: String;
...
linea:=ArrayDeLineas[lineaSeleccionada-1];
...
procedure TfImportador.BitBtnAltaEanClick(Sender: TObject);
var
  linea: RLinea;
  nEan: string;
begin
  ...
  ArrayDeLineas[linea.Pos-1]:=linea;
...
procedure TfImportador.BitBtnAceptarDatosBDClick(Sender: TObject);
var
  linea: RLinea;
  TxtQuery: String;
  datosDe: String;
...
//                   ' Descripción : ' + dbArti.FieldByName('ean2').AsString );
  ...
  //Las modificaciones pasan al vector de lineas de pedido
  ArrayDeLineas[linea.Pos-1]:=linea;
  //writeLinea(ArrayDeLineas[linea.Pos-1]);
...
procedure TfImportador.BitBtnDAltaCodClick(Sender: TObject);
var
...
procedure TfImportador.eCodigoExit(Sender: TObject);
begin
...
procedure TfImportador.eCodigoDesdeExit(Sender: TObject);
begin
  if not (EsNumero(eCodigoDesde.Text)) then begin eCodigoDesde.Text:=''; exit; end;
end;

procedure TfImportador.eEANExit(Sender: TObject);
begin
...
procedure TfImportador.eEANDesdeExit(Sender: TObject);
begin
  if not (EsNumero(eEANDesde.Text)) then begin eEANDesde.Text:=''; exit; end;
end;

procedure TfImportador.eNombreExit(Sender: TObject);
begin
...
procedure TfImportador.eNombreDesdeExit(Sender: TObject);
begin
  if not (EsNumero(eNombreDesde.Text)) then begin eNombreDesde.Text:=''; exit; end;
end;

procedure TfImportador.eCostoHastaExit(Sender: TObject);
begin
...
procedure TfImportador.eCostoDesdeExit(Sender: TObject);
begin
  if not (EsNumero(eCostoDesde.Text)) then begin eCostoDesde.Text:=''; exit; end;
end;

procedure TfImportador.eDecCostoHastaExit(Sender: TObject);
begin
...
procedure TfImportador.eDecCostoDesdeExit(Sender: TObject);
begin
  if not (EsNumero(eDecCostoDesde.Text)) then begin eDecCostoDesde.Text:=''; exit; end;
end;

procedure TfImportador.eDecPVPHastaExit(Sender: TObject);
begin
...
procedure TfImportador.eDecPVPDesdeExit(Sender: TObject);
begin
  if not (EsNumero(eDecPVPDesde.Text)) then begin eDecPVPDesde.Text:=''; exit; end;
end;

procedure TfImportador.eIVAHastaExit(Sender: TObject);
begin
...
procedure TfImportador.eIVADesdeExit(Sender: TObject);
begin
  if not (EsNumero(eIVADesde.Text)) then begin eIVADesde.Text:=''; exit; end;
end;

procedure TfImportador.ePVPHastaExit(Sender: TObject);
begin
...
procedure TfImportador.ePVPDesdeExit(Sender: TObject);
begin
  if not (EsNumero(ePVPDesde.Text)) then begin ePVPDesde.Text:=''; exit; end;
end;

procedure TfImportador.eUnidHastaExit(Sender: TObject);
begin
...
//i:integer;
//begin
//Write('longidud del vector ');writeLn(Length(ArrayDeLineas)-1);
//showmessage('Pinto todas las lineas');
//    for i:=0 to Length(ArrayDeLineas)-1 do
//  writeLinea(ArrayDeLineas[i]);
//end;

procedure TfImportador.FormClose(Sender: TObject; var CloseAction: TCloseAction);
begin
  CloseAction:=CaFree;
end;

procedure TfImportador.WriteLinea(const Linea: RLinea);
begin
...
procedure TfImportador.tsPendientesEnter(Sender: TObject);
begin
  dbgPendientes.Repaint;
end;

procedure TfImportador.tsProcesadosEnter(Sender: TObject);
begin
  dbgProcesados.Repaint;
end;

//procedure TfImportador.MostrarPosicion(Sender: TObject);
//begin
//  ePos.Text:=IntToStr(SynEdit1.CaretX);
//end;
