...
begin
  ...
  //ShowformImportar;
  FImportarPed.IniciaImportar(dbPedid, dbPedic);
end;
...
begin
  ...
  LabelTotal.Caption:=FormatFloat('###,###,##0.00',dbPedic.FieldByName('PC8').AsFloat);
  dbGrid2.SetFocus;
end;
...
begin
  ...
  Edit8.Text:=FormatFloat('0.000',dbPedid.FieldByName('PD10').AsFloat);//-- Costo
//  Edit31.Text:=FormatFloat('0.000',dbPedid.FieldByName('PD12').AsFloat);//- Precio venta(Sin Iva)
  ...
  Edit10.Text:=FormatFloat('0.000',dbPedid.FieldByName('PD15').AsFloat);//- Precio de costo (Con Iva)
  ...
  Edit13.Text:=FormatFloat('0.00',dbPedid.FieldByName('PD17').AsFloat);//-- Imp. Linea Costo + Impuestos
...
procedure TFPedido.Panel12Click(Sender: TObject);
begin

end;

procedure TFPedido.Panel13Click(Sender: TObject);
begin
...
begin
  ...
  Edit13.Text:=FormatFloat('0.00',StrToFloat(Edit7.Text)*StrToFloat(Edit10.Text));//--- Importe
end;
...
//  RecargoCosto:= (StrToFloat(Edit8.Text)*StrToInt(Edit18.Text) / 100);//- Recargo
 ...
 //---- Importe linea Costo + impuestos
  Edit10.Text:=FormatFloat('0.000',(StrToFloat(Edit8.Text)*(1 + StrToFloat(Edit9.Text)/100)));
 ...
 //---- P.V.P. si hay margen.
  if (StrToFloat(Edit11.text)<>0) and (StrToFloat(Edit8.text)<>0) then
    begin
      ...
      Edit31.Text:=FormatFloat('0.000',PrecioSin);//---- PVP sin IVA
...
begin
  ...
  //---- Importe linea Costo + impuestos
  RecargoCosto:=(StrToFloat(Edit8.Text)*StrToInt(Edit18.Text) / 100);//- Recargo
  Edit10.Text:=FormatFloat('0.000',(StrToFloat(Edit8.Text)*(1+ (StrToFloat(Edit9.Text)/ 100)) + RecargoCosto));
  Edit13.Text:=FormatFloat('0.00',StrToFloat(Edit7.Text)*StrToFloat(Edit10.Text));//--- Importe
...
begin
  ...
  Edit31.Text:=FormatFloat('0.000', PrecioSin);//-------- Precio sin iva
  Edit12.Text:=FormatFloat('0.000', PrecioCon);//------- Pvp
...
begin
  ...
  Edit31.Text:=FormatFloat('0.000',PvpSinIva);
...
begin
   if (Edit16.Text='') or (Edit16.Text='999999') then begin Edit16.Text:='999999'; exit; end;
   dbProve.Active:=False;
...
begin
  ...
  {$IFDEF LINUX}
    frReport1.LoadFromFile('/usr/share/facturlinex2/Report/ListaPedidos.lrf');
  {$ELSE}
    frReport1.LoadFromFile(Lee+'Report\ListaPedidos.lrf');
  {$ENDIF}
...
begin
  ...
  dbTrabajo.SQL.Text:='SELECT COUNT(*), SUM(PD8), SUM(PD8*PD10), SUM(PD17), SUM(PD18) FROM pedidd'+Tienda+' WHERE'+
  ...
  TxtQ:='UPDATE pedicc'+Tienda+' SET PC5='+dbTrabajo.Fields[0].AsString+', PC6='+dbTrabajo.Fields[1].AsString+
        ', PC7='+dbTrabajo.Fields[2].AsString+', PC8='+dbTrabajo.Fields[3].AsString+
        ', PC9='+dbTrabajo.Fields[4].AsString+
...
begin
  ...
  {$IFDEF LINUX}
    if RadioButton6.Checked=True then
      frReport1.LoadFromFile('/usr/share/facturlinex2/Report/Pedido.lrf');
    if RadioButton7.Checked=True then
      frReport1.LoadFromFile('/usr/share/facturlinex2/Report/PedidoSinPvp.lrf');
    if RadioButton8.Checked=True then
      frReport1.LoadFromFile('/usr/share/facturlinex2/Report/PedidoUnidades.lrf');
  ...
  {$ELSE}
    if RadioButton6.Checked=True then
      frReport1.LoadFromFile(Lee+'Report\Pedido.lrf');
    if RadioButton7.Checked=True then
      frReport1.LoadFromFile(Lee+'Report\PedidoSinPvp.lrf');
    if RadioButton8.Checked=True then
      frReport1.LoadFromFile(Lee+'Report\PedidoUnidades.lrf');
    if RadioButton20.Checked=True then
      frReport1.LoadFromFile(Lee+'Report\EtiLineales.lrf');
    //------------------------
    if Pdf=0 then
      frReport1.ShowReport
    else
...
begin
  ...
  SaveDialog1.FileName:=Lee + 'etieans.txt';
...
procedure TFPedido.Panel6Click(Sender: TObject);
begin
    ...
    begin
      ...
      while not dbPedid.EOF do
        begin
          ...
          if dbArti.RecordCount=0 then begin showmessage('SE HA BORRADO EL ARTICULO '+dbPedid.FieldByName('PD7').AsString); continue; end;
          dbArti.Edit;
          dbArti.FieldByName('A4').Value:=dbPedid.FieldByName('PD8').AsFloat;//--Unidades.
          dbArti.Post;
          dbPedid.Next;
        end;
    end;
