...
procedure TFLDepartamentos.ConfigurarBoton(ABoton: TBitBtn;
...
begin
  ...
  { El servidor genera el HEX después de convertir D1 a utf8mb4.
    Validamos y devolvemos los bytes como texto UTF-8 para LCL/GTK. }
  Result := UTF8Encode(UTF8Decode(Raw));
end;

procedure TFLDepartamentos.DescripcionHexGetText(Sender: TField;
...
procedure TFLDepartamentos.ConfigurarCampoDescripcion;
var
  Campo: TField;
begin
...
procedure TFLDepartamentos.ConfigurarCampoObservaciones;
var
  Campo: TField;
begin
  if not dbQuery.Active then Exit;
...
procedure TFLDepartamentos.ConfigurarGrid(AGrid: TDBGrid);
begin
...
procedure TFLDepartamentos.AplicarContrasteSeleccion(AControl: TWinControl);
{$IFDEF LCLGTK2}
var
...
begin
  ...
  begin
    gdk_color_parse(PChar('#DDDDDD'), @FondoNormal);
    ...
    gdk_color_parse(PChar('#DDDDDD'), @TextoSeleccion);
...
procedure TFLDepartamentos.AplicarContrasteSeleccionControles(AParent: TWinControl);
var
  I: Integer;
  C: TControl;
begin
...
procedure TFLDepartamentos.ConfigurarControlesRecursivo(AParent: TWinControl);
var
  I: Integer;
  C: TControl;
begin
...
procedure TFLDepartamentos.AplicarDisenoModerno;
begin
  Caption := 'Listados y análisis de departamentos';
  ...
  FLabelCabTitulo.Caption := 'LISTADOS Y ANÁLISIS DE DEPARTAMENTOS';
  ...
  FLabelCabSubtitulo.Caption :=
    'Consulte datos generales, compras o ventas agrupadas por departamento.';
  ...
  FLabelAyudaFiltros.Caption :=
    'Defina el intervalo de departamentos y, para compras o ventas, el ejercicio y los meses.';
  ...
  RadioButton1.Caption := 'Datos generales de departamentos';
  RadioButton2.Caption := 'Compras agrupadas por departamentos';
  RadioButton3.Caption := 'Ventas y beneficio por departamentos';
  ...
  ConfigurarBoton(BitBtn4, 'Cerrar',
    'Cerrar el formulario de departamentos',
...
procedure TFLDepartamentos.RecolocarControles;
var
...
procedure TFLDepartamentos.MostrarZonaFiltros;
begin
  FEnResultados := False;
...
procedure TFLDepartamentos.MostrarZonaResultados;
begin
  FEnResultados := True;
...
procedure TFLDepartamentos.ActualizarFlechaOrdenacion(AGrid: TDBGrid;
  AColumn: TColumn);
var
  I: Integer;
  Texto, Flecha: String;
...
procedure TFLDepartamentos.GridDrawColumnCell(Sender: TObject;
...
function TFLDepartamentos.GridResultadoVisible: TDBGrid;
begin
  Result := nil;
...
procedure TFLDepartamentos.PrepararCabeceraExportacion(ACabecera: TStrings);
var
  Grid: TDBGrid;
begin
  ...
  ACabecera.Add('Departamentos: códigos ' + Edit1.Text + ' a ' + Edit2.Text +
...
function TFLDepartamentos.GenerarPDFTemporal(out AFileName: String): Boolean;
var
  Grid: TDBGrid;
...
begin
  ...
  AFileName := IncludeTrailingPathDelimiter(GetTempDir(False)) +
    'facturlinex_departamentos_' + FormatDateTime('yyyymmdd_hhnnss_zzz', Now) + '.pdf';
...
procedure TFLDepartamentos.GuardarPDFClick(Sender: TObject);
var
  Dialogo: TSaveDialog;
  Grid: TDBGrid;
  Cabecera: TStringList;
begin
  ...
  Cabecera := TStringList.Create;
  try
    Dialogo.Title := 'Guardar listado de departamentos en PDF';
    Dialogo.Filter := 'Documento PDF (*.pdf)|*.pdf';
    Dialogo.DefaultExt := 'pdf';
    Dialogo.FileName := 'Listado_departamentos_' + FormatDateTime('yyyymmdd', Date) + '.pdf';
...
procedure TFLDepartamentos.PrevisualizarPDFClick(Sender: TObject);
var
...
procedure TFLDepartamentos.ImprimirPDFClick(Sender: TObject);
var
  Archivo, Mensaje: String;
begin
  if not GenerarPDFTemporal(Archivo) then Exit;
...
procedure TFLDepartamentos.ExportarCSVClick(Sender: TObject);
var
  Dialogo: TSaveDialog;
  Grid: TDBGrid;
begin
  ...
  Dialogo := TSaveDialog.Create(Self);
  try
    Dialogo.Title := 'Exportar listado de departamentos a CSV';
    Dialogo.Filter := 'Archivo CSV (*.csv)|*.csv';
    Dialogo.DefaultExt := 'csv';
    Dialogo.FileName := 'Listado_departamentos_' + FormatDateTime('yyyymmdd', Date) + '.csv';
    Dialogo.Options := Dialogo.Options + [ofOverwritePrompt];
    if not Dialogo.Execute then Exit;
    try
...
procedure TFLDepartamentos.AplicarFocoInicial(Data: PtrInt);
begin
...
procedure TFLDepartamentos.FormularioKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFLDepartamentos.FormularioResize(Sender: TObject);
begin
  RecolocarControles;
end;
...
procedure TFLDepartamentos.FormShow(Sender: TObject);
begin
...
begin
  { D1 y OBSER se mantienen intactos para los informes FastReport originales.
  ...
  DescripcionHexSQL := 'HEX(CONVERT(D1 USING utf8mb4))';
  ObservacionesHexSQL :=
    'HEX(CONVERT(MID(D4,1,30) USING utf8mb4))';
  ...
  if RadioButton1.Checked=True then
    begin
     ...
     TxtQuery := 'select D0, D1, ' + DescripcionHexSQL +
       ' AS DESCRIPCION_HEX, MID(D4,1,30) as OBSER, ' +
       ObservacionesHexSQL + ' AS OBSERVACIONES_HEX ';
 ...
 //--------------------------- COMPRAS POR DEPARTAMENTOS
  if RadioButton2.Checked=True then
    begin
     ...
     TxtQuery := 'SELECT D0, D1, ' + DescripcionHexSQL +
       ' AS DESCRIPCION_HEX, SUM(DD3) As Unid, SUM(DD4) As Impor';
  ...
  if RadioButton3.Checked=True then
    begin
     ...
     TxtQuery := 'SELECT D0, D1, ' + DescripcionHexSQL +
       ' AS DESCRIPCION_HEX, SUM(DD5) As Unid, SUM(DD6) As ImporPrc, ' +
       'SUM(DD7) As ImporCOS, SUM(DD6-DD7) As Benef';
     TxtQuery := TxtQuery + ' FROM departamentos'+Tienda+',estadepa'+Tienda+' WHERE';
     if Edit1.Text<>'' then
       TxtQuery:=TxtQuery + ' D0>='+Edit1.Text+' AND';
     if Edit2.Text<>'' then
...
begin
  ...
  RellenaListBoxAnos('SELECT DISTINCT(DD1) FROM estadepa' + Tienda,
