...
type
  ...
  TFLXRecibosPDFDocument = class
  private
    FPages: TList;
    FCurrent: TFLXRecibosPDFPage;
    FPageWidth: Double;
    FPageHeight: Double;
...
constructor TFLXRecibosPDFPage.Create;
begin
  inherited Create;
...
destructor TFLXRecibosPDFPage.Destroy;
begin
  Content.Free;
  inherited Destroy;
end;

constructor TFLXRecibosPDFDocument.Create;
begin
  inherited Create;
  FPages := TList.Create;
  FCurrent := nil;
...
destructor TFLXRecibosPDFDocument.Destroy;
var
  I: Integer;
begin
...
procedure TFLXRecibosPDFDocument.SetLandscape;
var
  Aux: Double;
begin
...
procedure TFLXRecibosPDFDocument.NewPage;
begin
  FCurrent := TFLXRecibosPDFPage.Create;
  FPages.Add(FCurrent);
end;

function TFLXRecibosPDFDocument.PageCount: Integer;
begin
  Result := FPages.Count;
end;

procedure TFLXRecibosPDFDocument.AddCommand(const ACommand: string);
begin
...
function TFLXRecibosPDFDocument.NumberToPDF(const AValue: Double): string;
begin
...
function TFLXRecibosPDFDocument.TextToHex(const AText: string): string;
var
...
function TFLXRecibosPDFDocument.TextWidthApprox(const AText: string;
...
procedure TFLXRecibosPDFDocument.SetFillColor(const R, G, B: Double);
begin
  AddCommand(NumberToPDF(R) + ' ' + NumberToPDF(G) + ' ' +
    NumberToPDF(B) + ' rg');
end;

procedure TFLXRecibosPDFDocument.SetStrokeColor(const R, G, B: Double);
begin
  AddCommand(NumberToPDF(R) + ' ' + NumberToPDF(G) + ' ' +
    NumberToPDF(B) + ' RG');
end;

procedure TFLXRecibosPDFDocument.SetLineWidth(const AWidth: Double);
begin
  AddCommand(NumberToPDF(AWidth) + ' w');
end;

procedure TFLXRecibosPDFDocument.FillRect(const X, Y, W, H: Double);
begin
  AddCommand(NumberToPDF(X) + ' ' + NumberToPDF(Y) + ' ' +
    NumberToPDF(W) + ' ' + NumberToPDF(H) + ' re f');
end;

procedure TFLXRecibosPDFDocument.StrokeRect(const X, Y, W, H: Double);
begin
  AddCommand(NumberToPDF(X) + ' ' + NumberToPDF(Y) + ' ' +
    NumberToPDF(W) + ' ' + NumberToPDF(H) + ' re S');
end;

procedure TFLXRecibosPDFDocument.Line(const X1, Y1, X2, Y2: Double);
begin
  AddCommand(NumberToPDF(X1) + ' ' + NumberToPDF(Y1) + ' m ' +
    NumberToPDF(X2) + ' ' + NumberToPDF(Y2) + ' l S');
end;

procedure TFLXRecibosPDFDocument.Text(const X, Y, ASize: Double;
...
procedure TFLXRecibosPDFDocument.TextRight(const XRight, Y, ASize: Double;
  const AText: string; ABold: Boolean);
begin
...
procedure TFLXRecibosPDFDocument.TextCenter(const XCenter, Y, ASize: Double;
  const AText: string; ABold: Boolean);
begin
...
procedure TFLXRecibosPDFDocument.AddPageNumbers;
var
  I, OldIndex: Integer;
  OldCurrent: TFLXRecibosPDFPage;
  Texto: string;
begin
  OldCurrent := FCurrent;
  ...
  try
    for I := 0 to FPages.Count - 1 do
    begin
      FCurrent := TFLXRecibosPDFPage(FPages[I]);
      SetFillColor(0.38, 0.42, 0.45);
      Texto := 'Página ' + IntToStr(I + 1) + ' de ' +
        IntToStr(FPages.Count);
      TextRight(FPageWidth - 28, 18, 8, Texto, False);
      Text(28, 18, 8, 'FacturLinEx - Recibo de facturas', False);
    end;
  finally
    if OldIndex >= 0 then
      FCurrent := TFLXRecibosPDFPage(FPages[OldIndex])
    else
      FCurrent := OldCurrent;
  end;
end;

procedure TFLXRecibosPDFDocument.WriteASCII(AStream: TStream;
...
procedure TFLXRecibosPDFDocument.SaveToFile(const AFileName: string);
var
  Stream: TFileStream;
...
begin
  ...
  try
    ...
    begin
      ...
      ContentData := AnsiString(TFLXRecibosPDFPage(FPages[I]).Content.Text);
