...
//-- Print QR Bixolon, junto a línea eliminada  PrinterDevice = '/dev/usb/lp1'; // Cambia esto si tu impresora usa otro
GS = #29;
ESC = #27;
  ...
  Procedure SendToPrinter(const Data: RawByteString);
  Procedure PrintQR(const QRData: String);
...
begin
    //----------------- CONEXION -----------------
    mysql_conectar(dbConect, DBHost, DBDataBase, DBUsuario, DBPasswd, DBProtocolo, StrToInt(DBPuerto));
...
for conta:=0 to Grid.Columns.Count-1 do
    begin Grid.Columns[conta].Color:= clBtnFace;
  end;
end;
...
//==============  PARA IMPRESIÓN QR EN IMPRESORAS DE TICKET BIXOLON AL MENOS ===============
procedure SendToPrinter(const Data: RawByteString);
var
  PrinterFile: THandle;
begin
  //-- Sustituido PrinterDevice por DevTicket
  PrinterFile := FileCreate(DevTicket);
  if PrinterFile = feInvalidHandle then
    raise Exception.Create('No se puede abrir la impresora en: ' + DevTicket);

  FileWrite(PrinterFile, Pointer(Data)^, Length(Data));
