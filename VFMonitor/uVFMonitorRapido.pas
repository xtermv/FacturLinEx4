...
procedure VFQR_OpenMonitor(const Conn: TZConnection; const Tienda: string);
...
type
  ...
  private
    ...
    LbHealth, LbTotal, LbCorrectos, LbPendientes, LbRevisar: TLabel;
...
begin
  if Conn = nil then
  begin
    ShowMessage('VF Monitor rápido: Conn=nil');
    Exit;
  end;
  if not Conn.Connected then
  begin
    ShowMessage('VF Monitor rápido: la conexión a BD no está conectada');
    Exit;
  end;

  F := TfrmVFQRapido.CreateWith(Conn, Tienda);
  F.Show;
end;

{ TfrmVFQRapido }

constructor TfrmVFQRapido.CreateWith(const Conn: TZConnection; const ATienda: string);
begin
  // Form sin .lfm -> CreateNew
  inherited CreateNew(nil, 1);

  Caption := 'Monitor VeriFactu rápido';
...
procedure TfrmVFQRapido.AssignButtonGlyph(ABtn: TBitBtn; AKind: Integer);
var
  B: TBitmap;
  C: TCanvas;
begin
...
procedure TfrmVFQRapido.CreateUI;
var
  HeaderRight: TPanel;
begin
  Color := clWhite;
  ...
  SummaryPanel.SetBounds(14, 86, TopPanel.ClientWidth - 28, 48);
  ...
  LbHealth := TLabel.Create(Self);
  LbHealth.Parent := SummaryPanel;
  LbHealth.SetBounds(14, 13, 245, 24);
  LbHealth.Caption := 'VERIFACTU: comprobando...';
  LbHealth.ParentFont := False;
  LbHealth.Font.Height := -13;
  LbHealth.Font.Style := [fsBold];
  LbHealth.Font.Color := clNavy;
  LbHealth.BringToFront;

  LbTotal := TLabel.Create(Self);
  LbTotal.Parent := SummaryPanel;
  LbTotal.SetBounds(280, 13, 120, 24);
  LbTotal.Caption := 'Registros: 0';
  LbTotal.ParentFont := False;
  LbTotal.Font.Style := [fsBold];

  LbCorrectos := TLabel.Create(Self);
  LbCorrectos.Parent := SummaryPanel;
  LbCorrectos.SetBounds(410, 13, 130, 24);
  LbCorrectos.Caption := 'Correctos: 0';
  LbCorrectos.ParentFont := False;
  LbCorrectos.Font.Style := [fsBold];

  LbPendientes := TLabel.Create(Self);
  LbPendientes.Parent := SummaryPanel;
  LbPendientes.SetBounds(550, 13, 145, 24);
  LbPendientes.Caption := 'Pendientes: 0';
  LbPendientes.ParentFont := False;
  LbPendientes.Font.Style := [fsBold];

  LbRevisar := TLabel.Create(Self);
  LbRevisar.Parent := SummaryPanel;
  LbRevisar.SetBounds(705, 13, 150, 24);
  LbRevisar.Caption := 'Revisar: 0';
  LbRevisar.ParentFont := False;
  LbRevisar.Font.Style := [fsBold];

  FilterGroup := TGroupBox.Create(Self);
  FilterGroup.Parent := TopPanel;
  FilterGroup.SetBounds(14, 142, TopPanel.ClientWidth - 28, 104);
  ...
  ActionPanel.SetBounds(14, 254, TopPanel.ClientWidth - 28, 40);
...
procedure TfrmVFQRapido.PopulateEstado;
begin
...
function TfrmVFQRapido.SafeTienda(const S: string): string;
var
  I: Integer;
begin
  Result := Trim(S);
...
function TfrmVFQRapido.HistTableName: string;
begin
  Result := 'hisopcc' + FTienda;
end;

function TfrmVFQRapido.ParseDateISO(const S: string; out D: TDateTime): Boolean;
var
  FS: TFormatSettings;
  SS: string;
begin
...
function TfrmVFQRapido.RowColorForEstado(const Estado: string): TColor;
begin
...
function TfrmVFQRapido.SortExprForField(const FieldName: string): string;
begin
...
procedure TfrmVFQRapido.RefreshData;
var
  SavedID: Int64;
  SortDir: string;
  SortExpr: string;
...
begin
  SavedID := 0;
  if (Q <> nil) and Q.Active and (not Q.IsEmpty) and
     (Q.FindField('id') <> nil) then
  ...
  Q.SQL.Text :=
    ...
    '  COALESCE(q.entorno,''SIN_CLASIFICAR'') AS entorno, '+
    '  COALESCE(q.origen,''ORIG'') AS origen, '+
    '  COALESCE(q.registro_uid,''ORIG'') AS registro_uid, '+
    '  CASE '+
...
procedure TfrmVFQRapido.RefreshSummary;
var
  B: TBookmark;
  Total, Correctos, Pendientes, Revisar: Integer;
  R, E: string;
begin
  Total := 0;
  Correctos := 0;
  ...
  Q.DisableControls;
  try
    if not Q.IsEmpty then B := Q.GetBookmark;
    Q.First;
    while not Q.EOF do
    begin
      Inc(Total);
      R := UpperCase(Trim(Q.FieldByName('vf_resultado').AsString));
      E := UpperCase(Trim(Q.FieldByName('estado').AsString));

      if R = 'CORRECTO' then
        Inc(Correctos)
      else if (E = 'PENDIENTE') or (E = 'EN_PROCESO') or
              (R = 'PENDIENTE REINTENTO') then
        Inc(Pendientes)
      else if (R = 'ACEPTADO CON ERRORES') or
              (R = 'INCORRECTO AEAT') or
              (R = 'ERROR') or
              (Pos('ERROR ', R) = 1) or
              (R = 'RESPUESTA INVALIDA') or
              (R = 'NO VERIFICADO') then
        Inc(Revisar);

      Q.Next;
    end;

    if (B <> nil) and Q.BookmarkValid(B) then
      Q.GotoBookmark(B);
  finally
    if B <> nil then Q.FreeBookmark(B);
    Q.EnableControls;
  end;

  LbTotal.Caption := 'Registros: ' + IntToStr(Total);
  LbCorrectos.Caption := 'Correctos: ' + IntToStr(Correctos);
  LbPendientes.Caption := 'Pendientes: ' + IntToStr(Pendientes);
  LbRevisar.Caption := 'Revisar: ' + IntToStr(Revisar);

  if Revisar > 0 then
  begin
    LbHealth.Caption := 'VERIFACTU: REQUIERE ACTUACIÓN';
    LbHealth.Font.Color := clMaroon;
    LbRevisar.Font.Color := clMaroon;
  end
  else if Pendientes > 0 then
  begin
    LbHealth.Caption := 'VERIFACTU: HAY ENVÍOS PENDIENTES';
    LbHealth.Font.Color := RGBToColor(160, 100, 0);
    LbRevisar.Font.Color := clBlack;
  end
  else
  begin
    LbHealth.Caption := 'VERIFACTU: TODO CORRECTO';
    LbHealth.Font.Color := clGreen;
    LbRevisar.Font.Color := clBlack;
  end;
end;

procedure TfrmVFQRapido.CbEstadoChange(Sender: TObject);
begin
...
procedure TfrmVFQRapido.SetupGridColumns;
...
begin
  ...
  try
    ...
    AddCol('numero', 'Nº', 60);
    AddCol('tipo_factura', 'Tipo', 62);
    AddCol('registro_uid', 'Registro', 92);
    AddCol('origen', 'Origen', 105);
    AddCol('entorno', 'Entorno', 105);
...
procedure TfrmVFQRapido.VFRespuestaGetText(Sender: TField; var aText: string; DisplayText: Boolean);
...
procedure TfrmVFQRapido.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TfrmVFQRapido.BtnRefreshClick(Sender: TObject);
begin
  FSortField := 'fecha';
  FSortDesc := True;
...
function TfrmVFQRapido.PromptReason(out ReasonText: string): Boolean;
var
  S: string;
begin
  S := '';
...
procedure TfrmVFQRapido.BtnRetryClick(Sender: TObject);
var
  Serie: string;
  Numero: Integer;
  Estado: string;
...
procedure TfrmVFQRapido.BtnDetailClick(Sender: TObject);
var
  Id: Int64;
  QD: TZQuery;
  Msg: string;
...
procedure TfrmVFQRapido.BtnCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmVFQRapido.GridDblClick(Sender: TObject);
begin
  BtnDetailClick(Sender);
end;

procedure TfrmVFQRapido.GridTitleClick(Column: TColumn);
var
  FN: string;
begin
...
procedure TfrmVFQRapido.GridDrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
var
  Bk: TColor;
  Estado: string;
