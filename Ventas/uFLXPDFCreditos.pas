...
type
  ...
  TFLXCreditosPDFDocument = class
  private
    FPages: TList;
    FCurrent: TFLXCreditosPDFPage;
    FPageWidth: Double;
    FPageHeight: Double;
...
constructor TFLXCreditosPDFPage.Create;
begin
  inherited Create;
...
destructor TFLXCreditosPDFPage.Destroy;
begin
  Content.Free;
  inherited Destroy;
end;

constructor TFLXCreditosPDFDocument.Create;
begin
  inherited Create;
  FPages := TList.Create;
  FCurrent := nil;
...
destructor TFLXCreditosPDFDocument.Destroy;
var
  I: Integer;
begin
...
procedure TFLXCreditosPDFDocument.SetLandscape;
var
  Aux: Double;
begin
...
procedure TFLXCreditosPDFDocument.NewPage;
begin
  FCurrent := TFLXCreditosPDFPage.Create;
  FPages.Add(FCurrent);
end;

function TFLXCreditosPDFDocument.PageCount: Integer;
begin
  Result := FPages.Count;
end;

procedure TFLXCreditosPDFDocument.AddCommand(const ACommand: string);
begin
...
function TFLXCreditosPDFDocument.NumberToPDF(const AValue: Double): string;
begin
...
function TFLXCreditosPDFDocument.TextToHex(const AText: string): string;
var
...
function TFLXCreditosPDFDocument.TextWidthApprox(const AText: string;
...
procedure TFLXCreditosPDFDocument.SetFillColor(const R, G, B: Double);
begin
  AddCommand(NumberToPDF(R) + ' ' + NumberToPDF(G) + ' ' +
    NumberToPDF(B) + ' rg');
end;

procedure TFLXCreditosPDFDocument.SetStrokeColor(const R, G, B: Double);
begin
  AddCommand(NumberToPDF(R) + ' ' + NumberToPDF(G) + ' ' +
    NumberToPDF(B) + ' RG');
end;

procedure TFLXCreditosPDFDocument.SetLineWidth(const AWidth: Double);
begin
  AddCommand(NumberToPDF(AWidth) + ' w');
end;

procedure TFLXCreditosPDFDocument.FillRect(const X, Y, W, H: Double);
begin
  AddCommand(NumberToPDF(X) + ' ' + NumberToPDF(Y) + ' ' +
    NumberToPDF(W) + ' ' + NumberToPDF(H) + ' re f');
end;

procedure TFLXCreditosPDFDocument.StrokeRect(const X, Y, W, H: Double);
begin
  AddCommand(NumberToPDF(X) + ' ' + NumberToPDF(Y) + ' ' +
    NumberToPDF(W) + ' ' + NumberToPDF(H) + ' re S');
end;

procedure TFLXCreditosPDFDocument.Line(const X1, Y1, X2, Y2: Double);
begin
  AddCommand(NumberToPDF(X1) + ' ' + NumberToPDF(Y1) + ' m ' +
    NumberToPDF(X2) + ' ' + NumberToPDF(Y2) + ' l S');
end;

procedure TFLXCreditosPDFDocument.Text(const X, Y, ASize: Double;
...
procedure TFLXCreditosPDFDocument.TextRight(const XRight, Y, ASize: Double;
  const AText: string; ABold: Boolean);
begin
...
procedure TFLXCreditosPDFDocument.TextCenter(const XCenter, Y, ASize: Double;
  const AText: string; ABold: Boolean);
begin
...
procedure TFLXCreditosPDFDocument.AddPageNumbers;
var
  I, OldIndex: Integer;
  OldCurrent: TFLXCreditosPDFPage;
  Texto: string;
begin
  OldCurrent := FCurrent;
  ...
  try
    for I := 0 to FPages.Count - 1 do
    begin
      FCurrent := TFLXCreditosPDFPage(FPages[I]);
      SetFillColor(0.38, 0.42, 0.45);
      Texto := 'Página ' + IntToStr(I + 1) + ' de ' +
        IntToStr(FPages.Count);
      TextRight(FPageWidth - 28, 18, 8, Texto, False);
      Text(28, 18, 8, 'FacturLinEx - Estado de cuenta de créditos', False);
    end;
  finally
    if OldIndex >= 0 then
      FCurrent := TFLXCreditosPDFPage(FPages[OldIndex])
    else
      FCurrent := OldCurrent;
  end;
end;

procedure TFLXCreditosPDFDocument.WriteASCII(AStream: TStream;
...
procedure TFLXCreditosPDFDocument.SaveToFile(const AFileName: string);
var
  Stream: TFileStream;
  ...
  try
    ...
    begin
      ...
      ObjText := '<< /Type /Page /Parent 2 0 R ' +
        '/MediaBox [0 0 ' + AnsiString(NumberToPDF(FPageWidth)) + ' ' +
        AnsiString(NumberToPDF(FPageHeight)) + '] ' +
        '/Resources << /Font << /F1 3 0 R /F2 4 0 R >> >> ' +
        '/Contents ' + AnsiString(IntToStr(ContentObj)) + ' 0 R >>';
      WriteObject(PageObj, ObjText);

      ContentData := AnsiString(TFLXCreditosPDFPage(FPages[I]).Content.Text);
