...
begin
  ...
  if FCon.Connect('ftp.passur.es', 21) then begin // if initial connect call worked
  ...
  if FCon.Authenticate(Name, Pass) then begin // if authentication with server passed
    ...
    s := '';
					...
					FCon.ChangeDirectory('httpdocs');
					Resultado:= TStringList.Create;
					// Nos aseguramos que termine en contrabarra
					sDirectorio := IncludeTrailingBackslash( '/home/juanma/prue/' );
				  ...
				  FindClose( Busqueda );
                  Cont := 0;					 
					while Cont < Resultado.Count do
						begin					
						 s := Resultado[Cont];//GetAnswer('Filename'); // see which file the user wants to PUT on the server
					     Cont := Cont+1;
					     if FileExists(sDirectorio + s) then // if it exits locally
                          begin
						  FCon.Put(sDirectorio + s); // then send it over
						  FCon.CallAction;
                  ...
                  //FCon.CallAction;
				  //FStado:=FCon.GetCurrentStatus;
	          //FStado:=FCon.CurrentStatus;
				    //if Length(s) > 0 then // if reason was given, write it
						//Write(s);
					FreeAndNil(FFile); // make sure not to leak memory

    while con< ((Resultado.Count*3)+5) do begin // main loop is here, for events and user interaction
